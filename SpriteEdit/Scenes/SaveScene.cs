...
namespace SpriteEdit.Scenes
{
    class SaveScene
    {
        ...
        public void Update()
        {
            if (Program.App.result == 1)
            {
                Program.App.result = -1;
                bool ok = false;
                ok = Sprite.TrySave(Text, Program.App.sprite);
                if (ok)
                {
                    Sprite.Save(Text, Program.App.sprite);
                    Program.App.SwitchScene(2, false);
                }
                else
                {
                    Program.App.LastScene = 3;
                    Program.App.SwitchScene(6, false);
                }
            }
            string TxtP1;
            string TxtP2;
            if (ConsoleKeyboard.KeyAvailable)
            {
                ...
                switch (ConsoleKeyboard.PressedKey)
                {
                    ...
                    case ConsoleKey.Enter:
                        ...
                        break;
                    case ConsoleKey.Escape: Program.App.SwitchScene(2, false); break;
        ...
        public void Draw()
        {
            Renderer.DrawWindow(new Rectangle(20, 7, 41, 11), "SAVE FILE", ConsoleColor.Blue, ConsoleColor.Gray, ConsoleColor.White);
            ...
            Renderer.DrawButton(new Rectangle(36, 13, 10, 3), "SAVE", ConsoleColor.Cyan, ConsoleColor.Black);
        }
    }
}
